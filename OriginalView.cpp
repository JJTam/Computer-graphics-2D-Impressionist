...
void OriginalView::draw()
{
	...
	if ( m_pDoc->m_ucBitmap ) 
	{
		...
		unsigned char* currentDisplay = m_pDoc->getCurrentOriginalImage();
		bitstart = currentDisplay + 3 * ((m_pDoc->m_nWidth * startrow) + scrollpos.x);
		...
		// cannot draw when outside of boundary
...
void OriginalView::setMarker(Point pt) {
	marker.x = pt.x;
	marker.y = pt.y;
	redraw();
}
...
void OriginalView::displayMarker() {
	glPointSize(5);
	glBegin(GL_POINTS);
		glColor3f(1.0, 0.0, 0.0);
		glVertex2f(marker.x, m_nWindowHeight - marker.y);
	glEnd();
}